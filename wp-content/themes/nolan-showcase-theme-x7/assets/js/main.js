/* Nolan Showcase Theme X7 — main JS (no jQuery) */
(() => {
	'use strict';
	...
	function initMobileMenu(menuRoot) {
		...
		const setOpen = (next) => {
			...
			if (isOpen) {
				// Move focus into the menu for accessibility, but do not trap.
		...
		// Reduce motion: open/close without transition delay.
	...
	function initNolanMenu() {
		...
		// If window is resized from mobile to desktop, close mobile menu.
