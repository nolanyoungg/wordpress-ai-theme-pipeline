<?php
/**
 * Nolan Showcase Theme X11 — theme bootstrap.
 *
 * Purpose:
 ...
 */
define( 'NOLAN_SHOWCASE_THEME_X11_VERSION', '1.0.0' );
 ...
 */
function nolan_showcase_theme_x11_asset_url( $relative_path ) {
 ...
 */
function nolan_showcase_theme_x11_asset_version( $relative_path ) {
	$path = get_theme_file_path( ltrim( (string) $relative_path, '/' ) );
	if ( $path && file_exists( $path ) ) {
		return (string) filemtime( $path );
	}
	return NOLAN_SHOWCASE_THEME_X11_VERSION;
}
 ...
 * @return void
 */
function nolan_showcase_theme_x11_setup() {
	...
	register_nav_menus(
		array(
			'primary' => __( 'Primary menu', 'nolan-showcase-theme-x11' ),
			'footer'  => __( 'Footer menu', 'nolan-showcase-theme-x11' ),
		)
	);
}
add_action( 'after_setup_theme', 'nolan_showcase_theme_x11_setup' );
 ...
 * @return void
 */
function nolan_showcase_theme_x11_content_width() {
	$GLOBALS['content_width'] = isset( $GLOBALS['content_width'] ) ? (int) $GLOBALS['content_width'] : 1200;
}
add_action( 'after_setup_theme', 'nolan_showcase_theme_x11_content_width', 0 );
 ...
 * @return void
 */
function nolan_showcase_theme_x11_widgets_init() {
	register_sidebar(
		array(
			'name'          => __( 'Sidebar', 'nolan-showcase-theme-x11' ),
			'id'            => 'sidebar-1',
			'description'   => __( 'Add widgets here.', 'nolan-showcase-theme-x11' ),
...
add_action( 'widgets_init', 'nolan_showcase_theme_x11_widgets_init' );
 ...
 * @return void
 */
function nolan_showcase_theme_x11_enqueue_assets() {
	$style_rel = 'assets/css/theme.css';
	$js_rel    = 'assets/js/theme.js';

	wp_enqueue_style(
		'nolan-showcase-theme-x11',
		nolan_showcase_theme_x11_asset_url( $style_rel ),
		array(),
		nolan_showcase_theme_x11_asset_version( $style_rel )
	);

	wp_enqueue_script(
		'nolan-showcase-theme-x11',
		nolan_showcase_theme_x11_asset_url( $js_rel ),
		array(),
		nolan_showcase_theme_x11_asset_version( $js_rel ),
		true
	);
...
add_action( 'wp_enqueue_scripts', 'nolan_showcase_theme_x11_enqueue_assets' );
 ...
 */
function nolan_showcase_theme_x11_get_brand_name() {
	return __( 'MNY Photo', 'nolan-showcase-theme-x11' );
}
 ...
 * @return void
 */
function nolan_showcase_theme_x11_site_logo_markup() {
	...
	$mark = nolan_showcase_theme_x11_asset_url( 'assets/images/logo-mark.svg' );
	...
	echo '<span class="nolan-menu__brand-text">' . esc_html( nolan_showcase_theme_x11_get_brand_name() ) . '</span>';
	echo '</a>';
}
 ...
 */
function nolan_showcase_theme_x11_get_contact_url() {
 ...
 * @return void
 */
function nolan_showcase_theme_x11_handle_contact_submission() {
	if ( 'POST' !== strtoupper( (string) ( $_SERVER['REQUEST_METHOD'] ?? '' ) ) ) {
		wp_safe_redirect( nolan_showcase_theme_x11_get_contact_url() );
		exit;
	}

	$redirect = wp_get_referer();
	if ( ! $redirect ) {
		$redirect = nolan_showcase_theme_x11_get_contact_url();
	}

	$nonce = isset( $_POST['nolan_contact_nonce'] ) ? (string) wp_unslash( $_POST['nolan_contact_nonce'] ) : '';
	if ( ! wp_verify_nonce( $nonce, 'nolan_showcase_theme_x11_contact' ) ) {
	...
	$subject = sprintf(
		...
		__( '[%1$s] New inquiry from %2$s', 'nolan-showcase-theme-x11' ),
...
add_action( 'admin_post_nolan_showcase_theme_x11_contact', 'nolan_showcase_theme_x11_handle_contact_submission' );
add_action( 'admin_post_nopriv_nolan_showcase_theme_x11_contact', 'nolan_showcase_theme_x11_handle_contact_submission' );
