/* Nolan Showcase Theme X7 — preview JS (mirrors theme menu behaviors; no jQuery) */
(() => {
	'use strict';
	...
	const isWithin = (el, container) => !!(el && container) && (el === container || container.contains(el));
	const setAriaExpanded = (btn, expanded) => btn && btn.setAttribute('aria-expanded', expanded ? 'true' : 'false');
	...
	function initDesktopMenu(menuRoot) {
		...
		const toggle = (key) => (openKey === key ? closeAll() : open(key));
	...
	function initMobileMenu(menuRoot) {
		...
		if (prefersReducedMotion()) mobileMenu.style.transition = 'none';
		return { close };
	}
